...
import type { Order, OrderResponse, OrderStats, OrderStatus, UpdateOrderStatusRequest } from '../types/order';
...
export class OrderService {
  ...
  static async getRestaurantOrders(restaurantId: string): Promise<Order[]> {
    const response = await fetch(`${API_BASE}/api/restaurants/${restaurantId}/orders`, {
      method: 'GET',
  ...
  static async getTodayOrders(restaurantId: string): Promise<Order[]> {
    const response = await fetch(`${API_BASE}/api/restaurants/${restaurantId}/orders/today`, {
      method: 'GET',
      headers: this.getAuthHeaders(),
    });
  ...
  static async getTableOrders(restaurantId: string, tableId: string): Promise<Order[]> {
    const response = await fetch(`${API_BASE}/api/restaurants/${restaurantId}/tables/${tableId}/orders`, {
      method: 'GET',
      headers: this.getAuthHeaders(),
    });
  ...
  static filterOrders(orders: Order[], filters: {
    status?: OrderStatus;
    table_id?: string;
    search?: string;
    date_from?: string;
  ...
  static sortOrders(orders: Order[], sortBy: 'created_at' | 'total_amount' | 'status' = 'created_at', sortOrder: 'asc' | 'desc' = 'desc'): Order[] {
